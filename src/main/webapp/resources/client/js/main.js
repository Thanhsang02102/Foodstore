(function ($) {
  ...
  $("#btnFilter").click(function (event) {
    ...
    if (targetArr.length > 0) {
      // URL encode each target value to handle special characters
      const encodedTargets = targetArr.map(t => encodeURIComponent(t));
      searchParams.set("target", encodedTargets.join(","));
    }
    if (customertargetArr.length > 0) {
      // URL encode each customertarget value to handle special characters
      const encodedCustomerTargets = customertargetArr.map(ct => encodeURIComponent(ct));
      searchParams.set("customertarget", encodedCustomerTargets.join(","));
    }
  ...
  if (params.has("target")) {
    ...
    targets.forEach((target) => {
      ...
      $(`#targetFilter .form-check-input[value="${decodedTarget}"]`).prop(
        "checked",
        true
      );
    });
  ...
  if (params.has("customertarget")) {
    ...
    targets.forEach((customertarget) => {
      const decodedCustomerTarget = decodeURIComponent(customertarget);
      $(
        `#customertargetFilter .form-check-input[value="${decodedCustomerTarget}"]`
      ).prop("checked", true);
    });
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    // Check if viewMoreBtn exists before accessing its properties
    if (!viewMoreBtn || viewMoreBtn.length === 0) {
      return;
    }

    const totalItems = document.querySelectorAll(".product-item");
    if (viewMoreBtn && totalItems.length < 8) {
