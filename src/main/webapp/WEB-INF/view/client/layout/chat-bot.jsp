    ...
    <body>
      ...
      <div class="chat-container" id="chatContainer">
        ...
        <div class="chat-input-container">
          <input type="text" class="chat-input" id="userInput" placeholder="Nhập câu hỏi..."
      ...
      <div class="chat-collapsed" id="chatCollapsed">
        ...
        <div class="chat-input-container" style="width: 90%; padding: 5px">
          <input type="text" class="chat-input" id="collapsedInput" placeholder="Nhập câu hỏi..."
            style="padding: 8px; width: 100%" />
          <button class="chat-send" onclick="sendCollapsedMessage()" style="margin-top: 5px; width: 100%">
            Gửi
          </button>
        </div>
      </div>
        ...
        async function fetchGeminiResponse(userText) {
          try {
            ...
            if (!response.ok) {
              ...
              // Nếu là lỗi server, vẫn thử lấy response
              if (response.status >= 500) {
                throw new Error("Lỗi server: " + response.status);
              }
              throw new Error(errorText || "Lỗi kết nối API: " + response.status);
            }

            const data = await response.text();
            console.log("API response received, length:", data ? data.length : 0);
            if (data && data.trim() !== "") {
              addBotMessage(data);
            } else {
              addBotMessage("Xin lỗi, tôi không thể trả lời câu hỏi này. Vui lòng thử lại sau.");
            }
          } catch (error) {
            // xóa thông báo đang nhập
            const typingIndicator = document.getElementById("typingIndicator");
            ...
            if (error.message.includes("API Gemini chưa được cấu hình")) {
              errorMessage = "Chatbot tạm thời không khả dụng. Vui lòng liên hệ quản trị viên để được hỗ trợ.";
